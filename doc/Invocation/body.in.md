...
The `--update` flag also uses Nancy’s `--delete` flag; when the output directory already exists, files in it that are not generated by Linton are deleted, except for hidden files (whose name starts with a period).
